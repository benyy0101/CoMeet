...
const followSlice = createSlice({
  ...
  reducers: {
    ...
    removeFollowing: (state, action: PayloadAction<string>) => {
      state.following = state.following.filter((f) => f.memberId !== action.payload);
    },
    removeFollowed: (state, action: PayloadAction<string>) => {
        state.followed = state.followed.filter((f) => f.memberId !== action.payload);
    }
  },
});
...
export const { setFollowed,setFollowing,addFollowed,addFollowing,removeFollowed,removeFollowing } = followSlice.actions;
