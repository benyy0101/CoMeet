...
export const modifyLounge = async (
  params: ModifyLoungeParams
): Promise<ModifyLoungeResponse> => {
...
export const deleteLounge = async (
  params: DeleteLoungeParams
): Promise<DeleteLoungeResponse> => {
  const { loungeId } = params;
  const url = `lounge/${loungeId}`;
